...
vector<float> StackedPlot::divide(const vector<float>& x, float y) {
    vector<float> out = x;
...
vector<float> StackedPlot::normalize(const vector<float>& x) {
    float summed = sum(x);
    return divide(x, summed);
}

vector<float> StackedPlot::accumulate(const vector<float>& x) {
    vector<float> out = x;
